...
! NEW long chain aromatics (1-ring & 2-ring systems)
  ...
  elseif (dabs(H_8+ 10) .lt. 1d-7) then ! H_8=-10: 1-dodecylnaphthalene: Naph-nC12
    ...
    CP_IG = A + B*T + C*T**2 + D*T**3 + E*T**4 + F*T**5 + G*T**6
!********************************************************************************
  elseif (H_8.lt.-10.5d0 .AND. H_8.gt.-23.5d0) then ! new data
    idx = - int(H_8) - 10
    A = coef(idx, 1)
    B = coef(idx, 2)
    C = coef(idx, 3)
...
!{
  ...
  !real(8) :: G, delta(n,n), coef_ab
  !real(8) :: delta(n,n), coef_ab
  ...
  !{
    ...
    call fugacities_only(P,T,n,Pc,Tc,w,type_k,x1,lnphi1,phi1)
    call fugacities_only(P,T,n,Pc,Tc,w,type_k,x2,lnphi1,phi2)
...
!{
  ...
  ! INPUT & OUTPUT
  integer :: n, n_miscible
  real(8) :: P,T,Pc(n),Tc(n),w(n),xL(n),xR(n),x1(n),x2(n)
  integer :: type_k(n)
  ! local variables
  real(8) :: phi1(n), phi2(n), lnphi1(n), lnphi2(n),   &
             x1_old(n), x2_old(n)    ! values of the last step
  integer :: i, j, jm, x1m, maxLoop=100000
  real(8) :: tol = 1d-7, K(n), A(n), sumA, sumB, xC(n)
  real(8) :: dx1(n), dx2(n), max_dx, sum_x1M, sum_x2M
  ...
  real(8) :: c1, c0
  real(8) :: am, bm ! NOT USED
  real(8) :: c0i, delta(n,n), G, coef_ab
  ...
  !{
    ...
    ! calculate initial fugacity
    !call fugacities(P,T,n,Pc,Tc,w,x1,type_k,coef_ab,phi1,G,delta)
    !call fugacities(P,T,n,Pc,Tc,w,x2,type_k,coef_ab,phi2,G,delta)
    call fugacities_only(P,T,n,Pc,Tc,w,type_k,x1,lnphi1,phi1)
    call fugacities_only(P,T,n,Pc,Tc,w,type_k,x2,lnphi1,phi2)
    ...
    do j=1,n-1
      ...
      if (x2(n).lt.0) then
        print *, 'negative', n, x1(n)
        x2(n) = - x2(n)
      end if

      x1(n) = x2(n)/K(n)
    end do
    !print *, 'New(xn):', x1(n), x2(n)
...
!    print *, 'C', i, (0.5*(x1+x2)-xR)/(xL-xR)
  ...
  print *, 'df:', x1*phi1-x2*phi2

  c0 = 0d0
  nc0 = 0
  do j=1,n
    if (dabs(x1(j)-x2(j))>1d-3) then
      c0i = (xC(j)-x2(j))/(x1(j)-x2(j))
      if (c0i>0) then
        c0 = c0 + c0i
        nc0 = nc0 + 1
      end if
...
!{
  ...
  integer :: n, stable
  real(8) :: P,T,Pc(n),Tc(n),w(n),z(n),h(n),Yi(n),Ye(n),y(n)
  integer :: type_k(n)
  ...
  do j=1,1008
    ...
    call fugacities_only(P,T,n,Pc,Tc,w,type_k,y,lnphi_y,fuga)
...
!{
  ...
  ! input and output variables
  integer :: n, stable
  real(8) :: P,T,Pc(n),Tc(n),w(n),z(n),lnK(n)
  integer :: type_k(n)
  ...
  call fugacities_only(P,T,n,Pc,Tc,w,type_k,z,lnphi_z,fuga)
  ...
  stable=1
  do j=1,4
     call phase_stability_iteration2(stablej,P,T,n,Pc,Tc,w,type_k,z,h,Yi(:,j),Ye,y)
     if (stablej == 0) then
        call fugacities_only(P,T,n,Pc,Tc,w,type_k,y,lnphi_y,fuga)
        dG=0.0d0
        do i=1,n
...
!{
  ...
  ! INPUT & OUTPUT
  integer :: n, n_miscible
  real(8) :: P,T,Pc(n),Tc(n),w(n),xL(n),xR(n),c1,c0,x1(n),x2(n)
  integer :: type_k(n)
  ! local variables
  real(8) :: phi1(n), phi2(n), lnphi1(n), lnphi2(n), lnK(n)
  ...
  call phase_stability2(stable,lnK,P,T,n,Pc,Tc,w,type_k,xC)
...
! Successive Substitution iterations
        ...
        call fugacities_only(P,T,n,Pc,Tc,w,type_k,x1,lnphi1,phi1)
        call fugacities_only(P,T,n,Pc,Tc,w,type_k,x2,lnphi2,phi2)
...
!{
  ...
  ! INPUT & OUTPUT
  integer :: n, n_miscible
  real(8) :: P,T,Pc(n),Tc(n),w(n),x1(n),x2(n)
  integer :: type_k(n)
  ! local variables
  real(8) :: phi1(n), phi2(n), lnphi1(n), lnphi2(n), lnK(n), K(n), lnK_old(n)    
