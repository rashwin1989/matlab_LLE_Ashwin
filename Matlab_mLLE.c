...
{
    double  *P, *T, *Pc, *Tc, *w, *xa, *xb, *xa0, *xb0, *xai, *xbi, *c, *c1;
    mwSize  rows, cols, n;
    ...
    */
    ...
    if(  mxGetM(prhs[4]) != rows || mxGetN(prhs[4]) != cols  
      ...
      || mxGetM(prhs[10]) != rows || mxGetN(prhs[10]) != cols )
      mexErrMsgTxt("5th to 11th inputs must have the same shape.");
    ...
    /*
    printf("xi1:");for (i=0; i<n; i++) printf("%9.2le ", xa0[i]);printf("\n");
    printf("xi2:");for (i=0; i<n; i++) printf("%9.2le ", xb0[i]);printf("\n");
    ...
    */
    ...
    species_lle3_(P,T,&n,Pc,Tc,w,ntk,xa0,xb0,c,c1,xa,xb,&n_miscible);
