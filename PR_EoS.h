...
!{
  ...
  call fugacities_general(P,T,n,Pc,Tc,w,x,type_k,coef_ab,V,lnphi,fuga,Gex,delta)
!}
end subroutine fugacities_only
...
subroutine pressure_PR_EoS2( &
    V, & ! volume (Unit: m^3/mol)
    T,   & ! temperature (Unit: K)
    n,   & ! number of species
    Pc,  & ! vector of critical pressures
    Tc,  & ! vector of critical temperatures
...
!{
  implicit none
  integer :: n
  real(8) :: V,P,T,Pc(n),Tc(n),w(n),x(n)
  integer :: type_k(n)
  real(8) :: coef_ab(n)
  ...
  P = R_gas*T/(V-bm) - am/(V*V+2d0*bm*V-bm*bm)
!}
end subroutine pressure_PR_EoS2
!***************************************************************************
...
!{
  ...
  save first_time, n_species, n_pure, A, B, G, r1_298, T_up, T_up2, kij_stored
...
!{
  ...
  integer :: n, stable, stable1, stable2, stable3, stable4
  real(8) :: P,T,Pc(n),Tc(n),w(n),z(n)
  integer :: type_k(n)

  ! local variables
  ! used
  integer :: i, j
  real(8) :: lnphi_z(n), h(n), Yi(n), Ye(n), y(n), K(n)
  real(8) :: lnphi_y(n), sumY
  ...
  Yi = K*z                                                          
  call phase_stability_iteration(stable1,P,T,n,Pc,Tc,w,type_k,h,Yi,Ye,y)    
                                                                    
  Yi = z/K                                                          
  call phase_stability_iteration(stable2,P,T,n,Pc,Tc,w,type_k,h,Yi,Ye,y)    
                                                                    
  Yi = dexp(h)                                                      
  call phase_stability_iteration(stable3,P,T,n,Pc,Tc,w,type_k,h,Yi,Ye,y)    
                                                                    
  Yi = (K*z + z/K + dexp(h))/3.0d0                                  
  call phase_stability_iteration(stable4,P,T,n,Pc,Tc,w,type_k,h,Yi,Ye,y)    

  stable = 1
  if (stable1==0 .or. stable2==0 .or. stable3==0 .or. stable4==0) stable = 0
...
subroutine phase_stability_iteration( &
    stable, & ! OUTPUT: 1 - stable; 0 - unstable
    P, & ! pressure (Unit: Pa)
    T, & ! temperature (Unit: K)
    n, & ! number of species
...
!{
  ...
  ! input and output variables
  integer :: n, stable
  real(8) :: P,T,Pc(n),Tc(n),w(n),h(n),Yi(n),Ye(n),y(n)
  integer :: type_k(n)
  ...
  stable = 1
  if (sumY1>one .or. sumY2>one) stable = 0
...
!}
end subroutine phase_stability_iteration
